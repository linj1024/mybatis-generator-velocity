...
//        }
        ...
        FullyQualifiedJavaType parameterTypes = FullyQualifiedJavaType
                .getNewListInstance();
        
        FullyQualifiedJavaType listType = new FullyQualifiedJavaType(
                introspectedTable.getBaseRecordType());
        parameterTypes.addTypeArgument(listType);

        Set<FullyQualifiedJavaType> importedTypes = new TreeSet<>();
//        importedTypes.add(parameterType);
        importedTypes.add(parameterTypes);
        importedTypes.add(new FullyQualifiedJavaType("org.apache.ibatis.annotations.Param"));
        method.addParameter(new Parameter(parameterTypes, "records","@Param(\"records\")")); //$NON-NLS-1$
        ...
        if (context.getPlugins().clientInsertBatchMethodGenerated(method, interfaze,
                introspectedTable)) {
